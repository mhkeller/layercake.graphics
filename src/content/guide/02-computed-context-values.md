...
Some of these properties get changed by the time they end up on the context object. For example, the [x](/guide#x-2), [y](/guide#y-2), [z](/guide#z-2) and [r](/guide#r-2) properties can be strings or arrays but when they're exposed on the context, they are always a function. Sometimes, it's useful to refer to those original props such as in the [Cleveland Dot Plot example](/example/ClevelandDotPlot), which uses the x accessor shorthand of providing a list of keys.

Or, [xDomain](/guide#xdomain-2) and the other domain props can be used to set manual limits on the min or max of the domain scale. This can be different from what gets set on the context [xDomain](/guide#xdomain-2) if the prop value contained any `null` values. If you want to refer to the original value for any reason, it's set on this object.
...
The calculated extent of the x-dimension of the data. This is the extent of the data taking into account any manual settings passed in for [xDomain](/guide#xdomain-2).
...
Same as [xDomain](/guide#xdomain-3) above but for the y domain.

### zDomain `Array:[min: Number, max: Number]`

Same as [xDomain](/guide#xdomain-3) above but for the z domain.

### rDomain `Array:[min: Number, max: Number]`

Same as [xDomain](/guide#xdomain-3) above but for the r domain.

### xRange `Array:[min: Number, max: Number]`

The range used for the x-scale. This is usually `[0, width]` unless it's been manually set via the [xRange](/guide#xrange-2) prop.

It's equivalent to calling `$xScale.range()`.

### yRange `Array:[min: Number, max: Number]`

Same as [xRange](/guide#xrange-3) above but for the y domain.

### zRange `Array:[min: Number, max: Number]`

Same as [xRange](/guide#xrange-3) above but for the z domain.

### rRange `Array:[min: Number, max: Number]`

Same as [xRange](/guide#xrange-3) above but for the r domain.
...
Same as [xGet](/guide#xget-1) but for the y scale.

### zGet(d: `Object`)

Same as [xGet](/guide#xget-1) but for the z scale.

### rGet(d: `Object`)

Same as [xGet](/guide#xget-1) but for the r scale.
