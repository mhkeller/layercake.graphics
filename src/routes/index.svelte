...
<div class="main">
	...
	<div id="dek">
		...
		<p>Read the <a href="guide" svelte:prefetch>guide</a> or try the <a href="https://github.com/mhkeller/layercake-template" target="_blank" rel="nofollow">starter template</a> that comes with example components. See the examples below and even edit them live. Here's a sample of what the code looks like:</p>
	</div>

	<div class="code-example">
		<pre>{@html hljs.highlight(codeExample, { language: 'html' }).value}</pre>
	</div>

	<div id="gallery">
		{#each examples as example}
			<div class="gallery-item">
				<h4 class="title"><a svelte:prefetch href="example/{example.slug}">{example.title}</a> <a class="edit-repl" href="https://svelte.dev/repl/{example.replPath}" target="_blank" rel="nofollow">Edit</a></h4>
	...
	<div id="ssr-gallery">
		{#each examplesSsr as example}
			<div class="gallery-item" class:scaled={example.title.toLowerCase().includes('map')}>
				<h4 class="title"><a svelte:prefetch href="example-ssr/{example.slug}">{example.title}</a> <a class="edit-repl" href="https://svelte.dev/repl/{example.replPath}" target="_blank" rel="nofollow">Edit</a></h4>
				<svelte:component this="{example.component}"/>
			</div>
		{/each}
	</div>
